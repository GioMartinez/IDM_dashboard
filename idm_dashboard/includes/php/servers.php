	...
	function getServers($dn,$tree){
		...
		if(!empty($alarms)){
			foreach($alarms as$alarmKey=>$alarmValue){
				...
				foreach($alarmValue['fields']['fields'] as$propertyKey=>$propertyValue){
					if($propertyValue['name']=="Description"){$tree[$alarmKey]['value']=$propertyValue['value'];}
					elseif(($propertyValue['name']=="service-key")||($propertyValue['name']=="nombre")){$tree[$alarmKey]['alarm']=$propertyValue['value'];}
				}
			}
		}
		return$tree;
